...
def refresh_risk_dashboard():
    ...
    # 2. Clear all data
    print("2⃣ CLEARING ALL DATA")
    print("-" * 30)
    
    # Clear checklist items
    checklist_deleted = ChecklistItem.objects.all().delete()
    print(f"  Deleted {checklist_deleted[0]} checklist items")
    
    # Clear audit logs
    audit_deleted = LogEntry.objects.all().delete()
    print(f"  Deleted {audit_deleted[0]} audit log entries")
    
    # Clear regulations
    regulations_deleted = RegulationUpdate.objects.all().delete()
    print(f"  Deleted {regulations_deleted[0]} regulations")
    ...
    print("3⃣ RELOADING HIPAA REGULATIONS")
    print("-" * 30)
