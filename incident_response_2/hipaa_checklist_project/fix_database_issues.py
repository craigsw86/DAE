...
def fix_database_issues():
    ...
    try:
        ...
        if orphaned_items.exists():
            print_status(f"⚠️ Found {orphaned_items.count()} orphaned checklist items", "WARNING")
        ...
        print_status("⏱️ Testing query performance...", "INFO")
        import time
        ...
        if query_time < 100:
            ...
            print_status("⚠️ Query performance: FAIR", "WARNING")
        else:
...
def create_missing_regulations():
    ...
    try:
        ...
        if RegulationUpdate.objects.count() == 0:
            print_status("⚠️ No regulations found, creating test regulations...", "WARNING")
            ...
            print_status(f"ℹ️ Found {RegulationUpdate.objects.count()} existing regulations", "INFO")
